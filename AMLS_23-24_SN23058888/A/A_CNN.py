...
class preprocessing_CNN(Dataset):
    ...
    # Noise Adding
    def add_noise(self, image, mean=0, std=10):
...
def A_CNN_training(train_images, train_labels, val_images, val_labels,
          num_epochs = 100, # Number of training epochs
          learning_rate = 1e-3, # Learning rate
          batch_size = 32, # Batch size
          num_classes = 2, # Number of output classes (normal or pneumonia)
          ):
    ...
    torch.save(model, os.path.join(root_path, "A_CNN_model.pth"))
    ...
    plt.savefig(os.path.join(root_path, "A_CNN_trainging_plot.png"))

    return model

# Test the CNN classifier
def A_CNN_testing(model, test_images, test_labels):
    # Inputs Preprocessing
